...
namespace DemoSqlite
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            var dbPath = Path.Combine(projectRoot, "DemoSqlite.db");
        ...
        static async Task RunDemoAsync(AppDbContext appDbContext, SmartDataContext dbContext)
        {
            ...
            Console.WriteLine("\nRunning SQL query with JOIN using ExecuteSqlQueryAsync extension...");
            var sqlQuery = @"
            ...
            Console.WriteLine("\nQuerying timeseries data for customer email...");
            var startTime = DateTime.UtcNow.AddHours(-1);
            var endTime = DateTime.UtcNow;
            var timeseriesData = await customers.GetTimeseriesAsync(customer1.Id.ToString(), "Email", startTime, endTime);
            Console.WriteLine($"Timeseries for Customer {customer1.Name} Email:");
            foreach (var data in timeseriesData)
            {
                Console.WriteLine($"  - {data.Timestamp}: {data.Value}");
            }
            ...
            Console.WriteLine("\nQuerying timeseries data for product description...");
            timeseriesData = await products.GetTimeseriesAsync(productToUpdate.Id.ToString(), "Description", startTime, endTime);
            Console.WriteLine($"Timeseries for Product {productToUpdate.Name} Description:");
            foreach (var data in timeseriesData)
            {
                Console.WriteLine($"  - {data.Timestamp}: {data.Value}");
            }
